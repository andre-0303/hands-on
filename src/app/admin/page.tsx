...
export default async function AdminPage() {
  ...
  return (
    <>
      <h1 className="font-display text-4xl font-bold tracking-tight">Hands ons</h1>
      {rows.length === 0 ? (
        <p className="mt-6 text-muted">
          Nenhum hands on ainda.{" "}
          <Link href="/admin/novo" className="text-orange-soft underline underline-offset-4">
            Gere o primeiro
          </Link>
        </p>
      ) : (
              ...
              {rows.map(({ h, area }) => (
                <tr key={h.id} className="border-b border-line">
                  <td className="py-3 pr-4">
                    <Link href={`/admin/${h.id}`} className="font-medium hover:text-orange">
                      {h.title}
                    </Link>
                  </td>
                  <td className="py-3 pr-4 text-muted">{area.name}</td>
                  <td className="py-3 pr-4 text-muted">{LEVEL_LABEL[h.level]}</td>
                  <td className="py-3 pr-4">
                    {h.status === "published" ? (
                      <Link href={`/${area.slug}/${h.slug}`} className="text-orange-soft hover:underline">
                        Publicado
                      </Link>
                    ) : (
                  ...
                  <td className="py-3 text-muted">{h.updatedAt.toLocaleDateString("pt-BR")}</td>
                </tr>
              ))}
            </tbody>
          </table>
